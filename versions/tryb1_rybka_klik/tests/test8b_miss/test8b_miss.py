"""
TEST 8b - Tracking rybki gdy gracz klika ZLE (MISS)

Cel: Zbadac jak tracking zachowuje sie gdy pojawiaja sie napisy MISS.
Napisy MISS sa artefaktami ktore moga zaklocac background subtraction
(dodatkowe jasne piksele = falszywi kandydaci na rybke).

Przebieg:
1. Nagrywa 500 klatek z okienka lowienia
2. GRACZ klika celowo w zle miejsce (generuje MISS)
3. Bot tylko obserwuje - NIE klika
4. Analiza: % detekcji w RED vs WHITE, wplyw MISS na tracking

Uruchomienie:
  python test8b_miss.py
  (wymaga uruchomionej gry z aktywna minigra lowienia)
  UWAGA: TY klikasz w ZLE miejsca zeby generowac MISS!
"""
...
OUTPUT_DIR = "test8b_miss"
FRAMES_DIR = os.path.join(OUTPUT_DIR, "frames")
NUM_FRAMES = 500
SAVE_EVERY_N = 5


def draw_debug_frame(frame, frame_nr, color, fish_pos):
...
def main():
    print("=" * 60)
    print("  TEST 8b - Tracking z MISS (gracz klika zle)")
    print("=" * 60)
    print()
    print("Upewnij sie ze:")
    print("  1. Gra jest uruchomiona")
    print("  2. Minigra lowienia jest aktywna")
    print("  3. TY klikasz CELOWO W ZLE MIEJSCE (generujesz MISS)")
    print("  4. Bot tylko obserwuje - NIE klika!")
    print()
    ...
    for i in range(NUM_FRAMES):
        ...
        if (i % SAVE_EVERY_N) == 0:
            display = draw_debug_frame(frame, i+1, color, fish_pos)
    ...
    print()
    print("=" * 60)
    print("  RAPORT - TEST 8b (tracking z MISS)")
    print("=" * 60)
    ...
    print(f"  Czas:                {elapsed:.1f}s ({total/elapsed:.1f} FPS)")
    print()
    print(f"  Fazy:")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"TEST 8b - Tracking z MISS ({time.strftime('%Y-%m-%d %H:%M:%S')})\n")
        f.write(f"{'='*60}\n\n")
