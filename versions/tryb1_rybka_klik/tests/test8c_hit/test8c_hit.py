"""
TEST 8c - Tracking rybki gdy gracz klika POPRAWNIE (HIT)

Cel: Zbadac jak tracking zachowuje sie gdy pojawiaja sie napisy HIT.
Napisy HIT sa artefaktami ktore moga zaklocac background subtraction
(podobnie jak MISS w test 8b).

Przebieg:
1. Nagrywa 500 klatek z okienka lowienia
2. GRACZ klika celowo W RYBKE (generuje HIT)
3. Bot tylko obserwuje - NIE klika
4. Analiza: % detekcji w RED vs WHITE, wplyw HIT na tracking

Uruchomienie:
  python test8c_hit.py
  (wymaga uruchomionej gry z aktywna minigra lowienia)
  UWAGA: TY klikasz W RYBKE zeby generowac HIT!
"""
...
def main():
    print("=" * 60)
    print("  TEST 8c - Tracking z HIT (gracz klika w rybke)")
    print("=" * 60)
    print()
    print("Upewnij sie ze:")
    print("  1. Gra jest uruchomiona")
    print("  2. Minigra lowienia jest aktywna")
    print("  3. TY klikasz CELOWO W RYBKE (generujesz HIT)")
    ...
    print()
    print("=" * 60)
    print("  RAPORT - TEST 8c (tracking z HIT)")
    print("=" * 60)
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"TEST 8c - Tracking z HIT ({time.strftime('%Y-%m-%d %H:%M:%S')})\n")
        f.write(f"{'='*60}\n\n")
